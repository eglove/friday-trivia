...
export default function paginationField(): FieldPolicy {
  return {
    ...
    read(existing = [], { args, cache }: any) {
      ...
      const count = data?._allQuizzesMeta?.count;
      const page = skip / first + 1;
      const pages = Math.ceil(count / first);

      // Check if we have existing items
      const items = existing.slice(skip, skip + first).filter((x: any) => x);
      // If
      // There are items
    ...
    merge(existing: any, incoming: any, { args }: any) {
      const { skip } = args;
      // This runs when the Apollo client comes back from the network with our item
